...
export function CoordinatorFacultyList({
  department,
}: CoordinatorFacultyListProps) {
  const [faculty, setFaculty] = useState<Faculty[]>([]);

  useEffect(() => {
    setFaculty(getFacultyByDepartment(department));
  }, [department]);
    ...
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Teachers assigned to {department} will appear here once added by
          admin.
        </p>
      </div>
    );
  ...
  return (
    <section className="flex max-h-[28rem] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="shrink-0 border-b border-slate-200 px-6 py-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {faculty.length} teacher{faculty.length === 1 ? "" : "s"} in{" "}
          {department}.
        </p>
      </div>
