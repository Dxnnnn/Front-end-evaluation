...
import { scoringScale, type SurveyQuestion } from "@/lib/types/survey-question";

interface SurveyQuestionsListProps {
  refreshKey: number;
}

export function SurveyQuestionsList({ refreshKey }: SurveyQuestionsListProps) {
    ...
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Add your first survey question using the form.
        </p>
      </div>
    );
  }
  ...
  return (
    <section className="flex max-h-[28rem] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="shrink-0 border-b border-slate-200 px-6 py-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          {questions.length} question{questions.length === 1 ? "" : "s"} on record.
        </p>
      </div>
